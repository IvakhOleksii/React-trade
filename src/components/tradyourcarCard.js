...
class TradeYourCarCard extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      <div className=" margin-top-medium">
        ...
        <Row className="trade-your-car-cards-container">
          {this.state?.cardsData?.cards?.map((item,index) => (
            <Col key={index} onClick={()=> this.handleOptionSelect(item?.value)} className="option-card-container  " lg={5} sm={12} md={5}>
...
export default TradeYourCarCard;
